...
# Funcion para obtener los componenentes cD(coeficientes de detalle de las altas frecuencias)
def decryption_dwt(signal, n_samples, len_data, scale):
    _, cD = pywt.dwt(signal[:n_samples], 'db12')
    recovered = cD[:len_data] * scale
    return recovered

# Funcion para obtener la señal sonificada que representa la imagen
...
plt.title('Imagen recuperada sin clave')s

plt.subplot(1, 3, 2)
plt.imshow(np.flipud(image_wrong_key, cmap ='gray', aspect ='auto')
plt.title('Imagen recuperada con la clave incorrecta')

plt.subplot(1, 3, 3)
plt.imshow(np.flipud(np.abs(image_good_key)),
           cmap ='gray', aspect ='auto')
plt.title('Imagen recuperada con la clave correcta')

plt.tight_layout()
plt.show()


"Codigo base del que se partio: Garza, B. Readme.org at master · bryangarza/spectre.
